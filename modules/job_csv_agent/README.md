...
预览只显示中文业务名称，分开呈现“已整理的岗位信息”“建议补充”和“仍未填写的重要信息”。每轮最多展示三个优先问题；已问过且尚未回答的问题不会原样重复，已识别的公司、岗位和用工类型不会再列为待澄清项。建议在用户确认前不会进入准备保存的字段。
...
当前模块测试覆盖 CRUD、预览与二次确认、API、数据文件与查询库同步、确定性意图继承、职责/要求语义拆分、招聘意图过滤、安全改写、问题去重、用户消息泄漏检查，以及建议确认前后的 Repository 写入边界。测试使用 fake interpreter 或确定性规则，不调用真实外部模型。
